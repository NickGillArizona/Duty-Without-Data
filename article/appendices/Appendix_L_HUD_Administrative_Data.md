...
**Cited by:** Note footnotes 26–27 (POSH per-program disability rates) and the enforcement-infrastructure apparatus (Appendix H § H.4 cross-references L.4).
**Evidentiary status:** Appendix-level administrative-data analysis; all inputs are public HUD / Data.gov releases.
**Regeneration:** `scripts/cdbg_analysis.py`, `scripts/posh_analysis.py`, `scripts/reac_analysis.py`; outputs in `results/{cdbg,posh,reac}_results.json` (see the file inventory at the end of this appendix).
...
HUD's CDBG accomplishment system uses activity codes organized into categories: housing (12+13+14A–14L+16A), public services (03T+05A–05Z), public improvements (03A–03Z), and economic development. Exactly two of these codes are disability-specific. None of the housing rehabilitation codes (14A–14L) records whether a given rehabilitation added accessibility features:
...
**FY2021 anomaly:** The 05B share spikes to 46.6% of public services in FY2021. That spike comes from a collapse in overall public service numbers, likely COVID-related, rather than any rise in disability spending. The 03B figure for FY2021 (249) is also anomalously low.

### Caveats

- Grantees may spend on disability accessibility under generic rehabilitation codes (14A–14L); the combined 2.2% disability share (2.51% of public-service and 1.32% of public-improvement beneficiaries) captures only spending coded specifically to disability. Actual disability-related spending may be higher but is unmeasurable in the CDBG accomplishment data.
...
| Section 811/PRAC | 33,664 | 97.7% | 30,432 |
...
- The "Est. Disabled Households" column is derived from age-disaggregated (under-62 / 62-plus) disability rates applied to program household counts, not from the displayed aggregate rate times the "Total Units" column; the two will not reconcile by simple multiplication.
...
The NSPIRE protocol transition is ongoing, with roughly 55% of inspections now run under NSPIRE. Failures detected jumped sharply once the new standard applied to the same housing stock, evidence that tightening the inspection standard, by itself, surfaces more deficiencies. The same transition opens a regulatory insertion point for an accessibility compliance module.
...
This figure counts only charges issued by HUD through FHEO, leaving out charges issued by FHAP state and local agencies. The NFHA 2025 Report shows 37 charges for 2019; the gap of 30 is the FHAP-agency cause determinations. HUD-direct charges averaged about 30 a year across 2014–2020, so the 2019 number may reflect internal processing delays or staffing patterns.
