...
**Coding protocol.** The same thirty-key output schema as the RA Database v4, with identical controlled vocabulary. The unified database contains canonical values for all fields, with an audit trail preserving all three model-specific values. (The published edition of the database omits five free-text and property-level fields under the archive's data-minimization policy; see `replication/DATA_PROVENANCE.md` § 2.5 and `scripts/minimize_public_dataset.py`.)
