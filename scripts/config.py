...
Required database files:
  - FHA_Unified_Database.json           (primary: 1,900 disability cases under T2 canonical filter)
...
def repo_path(p) -> str:
    """Return a repo-relative POSIX path, or '<external>/<basename>' if outside.

    Used by analysis runners when writing path strings into committed artifacts
    (manifests, methodology headers, status files). Keeps committed JSON/Markdown
    free of personal absolute paths so the repo is portable across machines.
    """
    abs_p = os.path.abspath(str(p))
    try:
        return os.path.relpath(abs_p, _REPO_ROOT).replace(os.sep, "/")
    except ValueError:
        # Different drive on Windows — relpath raises ValueError.
        return f"<external>/{os.path.basename(abs_p)}"
