...
outcome series reported in Part II on the universal 606 basis. Both must pass.
...
def check_case_level_series():
    ...
    UNIVERSAL 606 basis (results/series_2026-07.json)."""
    ...
    checks = [
        ("case-level N (287/68/251)",  cs["case_level_n_decided"],            [287, 68, 251]),
        ("case-level N pooled",        cs["case_level_n_pooled"],             606),
        ("qualifying judgments",       cs["distinct_qualifying_judgments"],   [10, 0, 8]),
        ("strict rate % (per window)", cs["strict_qualifying_rate_pct"],      [3.48, 0.00, 3.19]),
        ("strict rate % (pooled)",     cs["strict_qualifying_rate_pooled_pct"], 2.97),
        ...
        ("represented per window",     cs["represented_per_window"],          [116, 30, 60]),
        ("represented pooled",         cs["represented_pooled"],              206),
        ("pro se per window",          cs["pro_se_per_window"],               [171, 38, 191]),
        ("pro se pooled",              cs["pro_se_pooled"],                   400),
        ("represented cell 18/206",    [rvc["numerator"], rvc["denominator"]], [18, 206]),
        ("represented cell 8.7%",      rvc["pct"],                            8.7),
        ("pro se victories 0/400",     [psc["numerator"], psc["denominator"]], [0, 400]),
        ("pro se victories (zero)",    cs["pro_se_victories"],                [0, 0, 0]),
        ("pro se exact upper bound",   psc["exact_upper_bound_pct"],          0.918),
        ("docket shares 59.6->76.1",   cs["pro_se_docket_share_case_level_pct"],
                                                                             [59.6, 55.9, 76.1]),
        ("broad numerators",           cs["broad_favorable_numerators"],      [12, 0, 9]),
        ("broad favorable rate %",     cs["broad_favorable_rate_pct"],        [4.18, 0.00, 3.59]),
        ("P1-vs-P3 diff pp",           cs["p1_vs_p3_strict_diff_pp"],         -0.30),
        ...
        ("represented cell 95% CI",    rvc["ci95_pct"],                       [5.261, 13.458]),
        ("under-call bound 1/606",     [ucb["numerator"], ucb["denominator"]], [1, 606]),
        ("under-call upper bound %",   ucb["exact_upper_bound_pct"],          0.916),
        ("CP interval 18/606",         cp["18_606"],                          [1.7697, 4.6538]),
        ("CP interval 0/400",          cp["0_400"],                           [0.0, 0.918]),
        ("CP interval 1/606",          cp["1_606"],                           [0.0042, 0.916]),
    ]
    passes = fails = 0
    print()
...
def check_a7_case_level_text():
    ...
    required = [
        ...
        ("A-7 max shift +11.67pp",       "+11.67"),
        ("A-7 SJ cell 25.0% (29/116)",   "25.0% (29/116)"),
