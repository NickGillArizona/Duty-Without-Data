...
Three periods:
  ...
  P3: 2025-02-05 to 2026-07-01  (Post-HUD Secretary)

Produces: appendix_data.json  (machine-readable)
          appendix_report.md  (human-readable)
"""
import json, os, sys, math
from collections import Counter, defaultdict
from datetime import datetime
from config import UNIFIED_DB_PATH, RESULTS_DIR
# Canonical sample filters, imported rather than re-implemented locally: a local
# copy of the tier predicates drifts from the ones the rest of the archive applies
# (analysis_filters normalizes protected_classes case and whitespace).
...
P1_START = '2022-01-01'; P1_END = '2024-06-28'; P2_END = '2025-02-05'; P3_END = '2026-07-01'
DECIDED = DECIDED_OUTCOMES

def load(): return json.load(open(DB_PATH,'r',encoding='utf-8'))

# Canonical T2 on this snapshot is 1,900 records: screened-in AND (protected-class
# entry OR disability_alleged flag OR is_ra_case flag). The narrower filter
# (protected class only) yields 1,849 and is preserved as a sensitivity cohort
# (analysis_filters.is_t2_narrow).
has_disability = is_t2_canonical
...
def main():
    db = load()
    allsc = [r for r in db if is_screened_in(r)]
