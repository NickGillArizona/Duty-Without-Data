"""
Robustness Checks for "Duty Without Data" - FHA Unified Database
=====================================================================
...
P3_END   = "2026-07-01"   # database collection endpoint
...
def load_db():
    ...
    disab = [r for r in screened if (
        "disability" in [p.lower() for p in (r.get("protected_classes") or [])]
        or r.get("disability_alleged")
        or r.get("is_ra_case")
    )]
    return disab
...
def main():
    ...
    print("DUTY WITHOUT DATA -- EMPIRICAL ROBUSTNESS CHECKS (document-level diagnostics)")
    print("Run date: {}".format(datetime.now().strftime("%Y-%m-%d %H:%M")))
    print("Database: data/FHA_Unified_Database.json")
    print()
