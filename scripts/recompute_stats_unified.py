"""Recompute three-period litigation statistics on the FHA Unified Database.

Default filter is the Note's canonical T2-disjunctive disability population:
screened-in FHA records with a disability protected-class label, independent
disability-alleged flag, or Rehabilitation Act / reasonable-accommodation flag.
Use ``--narrow`` to run the protected-class-only robustness sample.
"""
...
from analysis_filters import (
    ...
    assign_period as canonical_period,
    is_screened_in,
    is_t2_canonical,
    is_t2_narrow,
)
...
NARROW_STATS_OUTPUT = os.path.join(RESULTS_DIR, 'unified_stats_t2_narrow.json')
NARROW_REPORT_OUTPUT = os.path.join(RESULTS_DIR, 'unified_stats_t2_narrow_report.md')
...
def main():
    parser = argparse.ArgumentParser(description="Recompute FHA disability litigation statistics.")
    parser.add_argument(
        "--narrow",
        action="store_true",
        help="Use T2-narrow robustness filter (protected_classes contains disability only).",
    )
    args = parser.parse_args()

    db = load_json(UNIFIED_DB)
    disability_filter = is_t2_narrow if args.narrow else is_t2_canonical
    stats_output = NARROW_STATS_OUTPUT if args.narrow else STATS_OUTPUT
    report_output = NARROW_REPORT_OUTPUT if args.narrow else REPORT_OUTPUT
    filter_label = "T2-narrow robustness" if args.narrow else "T2 canonical disjunctive"
    filter_description = (
        'screened-in AND "disability" in protected_classes'
        if args.narrow
        else 'screened-in AND (disability_alleged OR is_ra_case OR "disability" in protected_classes)'
    )

    # All screened-in
    all_screened = [r for r in db if is_screened_in(r)]
    # Disability filter
    disab = [r for r in db if disability_filter(r)]

    for r in disab:
        r['_period'] = canonical_period(r)
    ...
    print(f"Disability cases ({filter_label}): {len(disab)} ({100*len(disab)/len(all_screened):.1f}%)")
...
**Database:** FHA Unified Database, filtered to disability cases ({filter_label})
- Filter expression: `{filter_description}`
