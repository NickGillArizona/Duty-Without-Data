...
NONCOMPLIANCE_RATE_SOURCE = (
    ...
    "article/appendices/Appendix_H_Supplementary_Data.md"
)
...
def require_curl() -> str:
    ...
    if not curl_path:
        raise RuntimeError("curl is required for stable Census API queries in this environment")
    return curl_path
...
def build_literal_four_plus_bounds(
    ...
    return {
        ...
        "exact_size_gap_status": (
            "Public Census products do not point-identify exact 4-unit buildings; "
...
Note: tenure is constant because the analysis universe is renter-occupied households. The `3-4 units` rows use the ACS PUMS `BLD=05` bucket, which necessarily includes some 3-unit structures.
