...
def load_national_data():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        # The household estimate uses the UNROUNDED rate (the appendix table's
        # column convention); the rate is rounded to 1dp for display only.
        share_62plus = pct_age62plus / 100 if pct_age62plus else 0
        share_under62 = 1 - share_62plus
        weighted_rate_raw = pct_dis_lt62 * share_under62 + pct_dis_ge62 * share_62plus
        weighted_rate = round(weighted_rate_raw, 1)

        est_disabled_hh = round(number_reported * weighted_rate_raw / 100)
