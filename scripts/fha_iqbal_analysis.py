...
======================================================================
Analyzes the FHA Unified Database (primary_protected_class == "disability")
to quantify the structural pleading barrier facing disability plaintiffs in
complaint-driven enforcement. Console-only diagnostic; writes no artifact.
Schema note: outcome/procedural_posture enums are uppercase (e.g.
MOTION_TO_DISMISS, DEFENDANT_WIN); the decision year is the `year` field
(`year_decided` is unset in the current build).
"""
...
for c in dis_mtd_failed_sub:
    print(f"    - {c['case_name'][:70]}  ({c.get('year')}, {c.get('disability_category','')})")
