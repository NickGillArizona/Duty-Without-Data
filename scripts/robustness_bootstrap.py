"""
Bootstrap confidence intervals (10,000 resamples, seed 42, preserving period
assignment and representation status) for the document-level decomposition
diagnostics produced by scripts/decomposition.py.

Document-level pipeline diagnostic; not reported in the manuscript. See
replication/REPRODUCE.md (document-level pipeline diagnostics) for the boundary.

Periods are keyed to the two external shocks in the Note's three-period design:
    P1 : 2022-01-01  <=  date_filed  <  2024-06-28   (pre-Loper Bright baseline)
    P2 : 2024-06-28  <=  date_filed  <  2025-02-05   (transition)
    P3 : 2025-02-05  <=  date_filed  <=  2026-07-01  (post-withdrawal)
...
from config import UNIFIED_DB_PATH, RESULTS_DIR, repo_path
from analysis_filters import (
    NON_DECISIVE_OUTCOMES as EXCLUDED_OUTCOMES,
    P1_START,
    P2_START,
    P3_START,
    P3_END,
...
def load_disability_cases() -> pd.DataFrame:
    ...
    cases = [r for r in raw if is_t2_canonical(r)]
    ...
    for r in cases:
        ...
        if not raw_date:
            continue
        d = parse_opinion_date(raw_date)
        if d is None:
            continue
        if d < P1_START or d > P3_END:
            continue
        if d < P2_START:
            period = "P1"
        elif d < P3_START:
...
def main() -> None:
    ...
    print(f"  P3 (2025-02-05 to 2026-07-01): {(df.period == 'P3').sum()}")
    ...
    out = {
        "metadata": {
            ...
            "unified_db_path": repo_path(UNIFIED_DB_PATH),
        },
