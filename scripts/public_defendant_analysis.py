...
Scope note (2026-07-06): the disability-wave classification merge this script joins
to is a PRE-EXTENSION artifact (an optional local input that is not distributed with
the public checkout and does not include the July-2026 refresh cases). The committed results/ outputs and
supplementary/empirical_extensions.md section 4 preserve the original 413-case public-defendant-
process analysis. The inline selector below approximates the original
unified_raw_text_target_inventory.py bucket; on the current 3,366-record database it
matches a larger universe, so re-running this script produces a different
(extension-inclusive, partly uncoded) selection and should not overwrite the committed
413-case outputs without a deliberate refresh decision.
"""
...
_REPO_ROOT_FOR_EMIT = _osp.dirname(_osp.dirname(_osp.abspath(__file__)))
def _emit_repo_path(p):
    abs_p = _osp.abspath(str(p))
    try:
        return _osp.relpath(abs_p, _REPO_ROOT_FOR_EMIT).replace(_os.sep, "/")
    except ValueError:
        return f"<external>/{_osp.basename(abs_p)}"
...
def main() -> None:
    ...
    if len(selected_records) != 413:
        print(
            f"NOTE: matched {len(selected_records)} public-defendant-process cases; the originally "
            "documented universe was 413. The inline selector approximates the original "
            "unified_raw_text_target_inventory.py bucket and the July-2026 refresh enlarged the "
            "database, so this count is expected to differ. Proceeding on the current selection "
    ...
    results: Dict[str, Any] = {
        ...
        "input_files": {
            "db_path": _emit_repo_path(DB_PATH),
            "merged_path": _emit_repo_path(MERGED_PATH),
        },
        "selection_method": {
            "selection_label": "public_defendant_process_cases",
            "selection_source": "Replicates scripts/unified_raw_text_target_inventory.py bucket logic to match the 413-case universe.",
            "selection_rule": {
