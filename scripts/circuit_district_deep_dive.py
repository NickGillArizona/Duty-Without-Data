...
def is_disability(case: dict) -> bool:
    ...
    if case.get("screening_result") != "YES":
        return False
    return (
        case.get("primary_protected_class") == "disability"
        or "disability" in [p.lower() for p in (case.get("protected_classes") or [])]
        or case.get("disability_alleged")
        or case.get("is_ra_case")
    )
...
def build_memo(results: dict) -> str:
    ...
    lines.append("- Appointment dates come from manual seeds first, then any prior district-keyed lookup results already cached in results/circuit_district_deep_dive_results.json when that file exists, and only then from live Wikipedia lookups. That makes reruns reproducible within this environment, but a fresh machine or later Wikipedia edits can still change the unresolved/resolved mix at the margins.")
    lines.append("")
    ...
    lines.append("- The appointment-lookup layer is partly path-dependent: if a prior deep-dive JSON exists in this environment, the script reuses those resolved biographies before hitting live Wikipedia. That improves stability inside this repository but should be disclosed in replication notes.")
