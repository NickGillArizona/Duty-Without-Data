#!/usr/bin/env python3
"""Document-level mechanism analysis (as-run). Several required run inputs are
author-held and not distributed with this archive; the committed output of record
is results/pro_se_mechanism_divergence_results.json."""
...
def repo_path(path: Path) -> str:
    """Return a repo-relative POSIX path string for committed manifest fields."""
    abs_p = Path(path).resolve()
    try:
        return abs_p.relative_to(repo_root()).as_posix()
    except ValueError:
        return f"<external>/{abs_p.name}"
...
def has_disability(record: Dict[str, Any]) -> bool:
    """Canonical T2 (1,771) per Appendix M.1.1: protected-class entry OR
    disability_alleged flag OR is_ra_case flag."""
    classes = record.get("protected_classes") or []
    if isinstance(classes, str):
        classes = [classes]
    return (
        any("disability" in str(item).lower() for item in classes)
        or bool(record.get("disability_alleged"))
...
def build_deduped_lookup_artifact(
    ...
    return {
        ...
        "raw_lookup_path": repo_path(raw_lookup_path),
...
def build_full_build_audit(
    ...
    return {
        ...
        "path_consistency": {
            "repo_local_data_path": repo_path(repo_data_path),
            "merge_artifact_data_path": repo_path(external_data_path),
        ...
        "artifacts": {
            "build_audit_path": repo_path(build_audit_path),
            "deduped_lookup_path": repo_path(deduped_lookup_path),
        },
        "notes": [
...
def build_memo(results: Dict[str, Any]) -> str:
    ...
    memo = [
        ...
        "- Some merge artifacts point to a non-repo staging copy instead of repo-local `data/FHA_Unified_Database.json`; the build audit records that those inputs are SHA-256 identical, so the difference is path drift rather than data drift.",
...
def main() -> None:
    ...
    results = {
        ...
        "inputs": {
            ...
            "external_data_path": repo_path(external_data_path),
            "combined_merged_path": repo_path(combined_merged_path),
            "combined_results_path": repo_path(combined_results_path),
            "full_lookup_path": repo_path(full_lookup_path),
            "wave_results_path": repo_path(wave_results_path),
            "gap_results_path": repo_path(gap_results_path),
            "gap_lookup_path": repo_path(gap_lookup_path),
            "gap_base_lookup_path": repo_path(gap_base_lookup_path),
            "gap_rerun_lookup_path": repo_path(gap_rerun_lookup_path),
        },
