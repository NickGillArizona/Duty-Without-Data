...
def resolve_schema_path() -> Path:
    extra = [
        REPO_ROOT / "method" / "pipeline" / "unified_overnight_schema.md",
    ]
...
def resolve_examples_path() -> Path:
    extra = [
        REPO_ROOT / "method" / "pipeline" / "unified_overnight_examples.json",
    ]
...
def build_requests(args: argparse.Namespace) -> Tuple[List[Dict[str, Any]], Dict[str, Any], Dict[str, Any]]:
    ...
    manifest = {
        ...
        "workspace_root": _emit_repo_path(NOTE_ROOT),
        "repo_root": _emit_repo_path(REPO_ROOT),
        "input_db": _emit_repo_path(input_db),
        "schema_path": _emit_repo_path(schema_path),
        "examples_path": _emit_repo_path(examples_path),
...
async def run_requests(
    ...
    if not api_key:
        raise ConfigError("OPENROUTER_API_KEY is not set. Set it in the environment or in a .env file at the repository root.")
