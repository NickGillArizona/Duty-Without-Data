...
```mermaid
flowchart LR
    CL[CourtListener<br/>REST API v4<br/>Free Law Project]
    SUB[Subscription database<br/>docket cross-checks]
    PA[PACER<br/>docket captures<br/>U.S. Courts]
    CL -->|opinion text +<br/>docket metadata| DEDUP{Deduplicate<br/>by source_file hash}
    SUB -->|decision_date<br/>citation only| DEDUP
    PA -->|standing-order<br/>disposition fields| DEDUP
    DEDUP --> RAW[Raw unified corpus<br/>T0 = 3,366]
    RAW --> SCR{Screening<br/>fha_screening.md}
    SCR -->|YES| T1[T1 = 2,690<br/>screened-in FHA]
    T1 --> CLF[Multi-model<br/>consensus pipeline]
    CLF --> DB[(FHA_Unified_Database.json)]
    style CL fill:#e8f8e8
    style SUB fill:#fff0e0
    style PA fill:#fff0e0
    style DB fill:#fff4e0
```
