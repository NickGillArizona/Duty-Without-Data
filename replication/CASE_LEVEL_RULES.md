...
to 595 distinct decided case units. 116 units contain more than one kept row (251
rows in total; 135 rows are absorbed into their unit); 479 units are singletons.
Eight units carry split-format identifiers (`FHV2SPLIT02`, `FHV2SPLIT03`;
`FHQ5SPLIT01`-`FHQ5SPLIT04`; `FHW1BSPLIT01`, `FHW1BSPLIT02`) from the successive
harmonization audits; all but `FHW1BSPLIT02` (a two-row unit) are singletons.
