...
**Reproducibility boundary.** Every published analysis reproduces from the frozen
`FHA_Unified_Database.json` committed here. Reconstructing that file itself is NOT supported from a
clean clone: `scripts/build_unified_db.py` consumes an upstream resolved database
(`FHA_RESOLVED_DB_PATH`) produced by the multi-model classification pipeline, which is retained in
the author's working archive. The two source corpora and the pipeline documentation are committed
so the construction is auditable; the frozen canonical JSON is the replication baseline. See
`REPRODUCE.md` ("What is deterministic and what is not").

## Committed in this repo

| Path | Source | Used by |
|---|---|---|
| `FHA_Unified_Database.json` | Built from `FHA_RA_Database_unified_*.json` + `FHA_3604_Database_unified_*.json` via [`scripts/build_unified_db.py`](../scripts/build_unified_db.py) (which consumes a resolved DB produced by the upstream three-model classification pipeline; see [`pipeline/model_metadata.json`](../method/pipeline/model_metadata.json) and [`pipeline/consensus_resolution.md`](../method/pipeline/consensus_resolution.md)) | All hypothesis-test scripts (`h1_h2_analysis.py`, `h5_analysis.py`, `h6_analysis.py`, `h7_analysis.py`, `h8_analysis.py`), `robustness_checks.py`, `supplemental_batch.py` |
...
| `The-Kelsey-504-ANPRM-Comments.{pdf,txt}` | The Kelsey (public comments on HUD 504 ANPRM) | Background reference for § 504 ANPRM analysis (general source material; no current Note appendix relies on this) |
